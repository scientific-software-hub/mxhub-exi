<div id="{.id}-div">
	<svg id="{.id}-svg"  height="{@math key="{mainRadius}" method="multiply" operand="2.2" /}" width="{@math key="{mainRadius}" method="multiply" operand="2.2" /}" style="overflow:visible">
		...
		{#cells}
				{?enableClick}{?.dataCollectionIds}<a id="{.id}-anchor" href='#/mx/datacollection/datacollectionid/{.dataCollectionIds}/main' target="_blank" style="target-new: tab;">{/.dataCollectionIds}{/enableClick}
