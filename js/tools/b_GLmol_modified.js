...
var GLmol = (function () {
    /** Changed for Al. **/
    function GLmol(id, suppressAutoload, force2d, pdb) {
        if (pdb) this.pdb = pdb;
    ...
    GLmol.prototype.create = function (id, suppressAutoload, force2d) {
        ...
        this.ElementColors = {
            ...
            "FE": 0xCC6600, "CA": 0x8888AA, "XX": 0xFFFF00
        };
    ...
    GLmol.prototype.parseXYZ = function (str) {
        try {
            /** Modefied block by al. 
       var atoms = this.atoms;
       var protein = this.protein;
    
       var lines = str.split("\n");
       if (lines.length < 3) return;
       var atomCount = parseInt(lines[0].substr(0, 4));
       if (isNaN(atomCount) || atomCount <= 0) return;
       ...
       **/
            ...
            var lines = str.split("\n");
            if (lines.length < 3) return;
            var atomCount = parseInt(lines.length) - 1;
            //if (isNaN(atomCount) || atomCount <= 0) return;
            //if (lines.length < atomCount + 2) return;
            var offset = 2;
    ...
    GLmol.prototype.addSuperpositionXYZ = function (XYZ, color, opacity, radius) {
        if (color == null) {
            color = "0xFFCCFF";
        }
        if (opacity == null) {
        ...
        if (XYZ != null) {
            ...
            if (lines.length > 3) {
                for (var i = 2; i < lines.length - 1; i++) {
                    ...
                    var sphereMaterial = new THREE.MeshLambertMaterial({ color: color, opacity: opacity });
    ...
    //GLmol.prototype.addThings = function() {
    //	for (var i = 0; i < 1000; i++) {
    //		var sphereGeometry = new THREE.SphereGeometry(1, this.sphereQuality, this.sphereQuality)
    //		var sphereMaterial = new THREE.MeshLambertMaterial({color: "0xFFCCFF"});
    //		 var sphere = new THREE.Mesh(sphereGeometry, sphereMaterial);
    //		  var sphereGeometry = new THREE.SphereGeometry(1, this.sphereQuality, this.sphereQuality); // r, seg, ring
    ...
    GLmol.prototype.drawAtomsAsSphere = function (group, atomlist, defaultRadius, forceDefault, scale) {
        ...
        if (this.atoms[atomlist[0]] != null) {
            if (this.atoms[atomlist[0]].radius != null) {

                sphereGeometry = new THREE.SphereGeometry(this.atoms[atomlist[0]].radius, this.sphereQuality, this.sphereQuality); // r, seg, ring
            }
        }
        for (var i = 0; i < atomlist.length; i++) {
            ...
            if (atom.opacity != null) {
                sphereMaterial = new THREE.MeshLambertMaterial({ color: atom.color, opacity: atom.opacity });
            }
    ...
    GLmol.prototype.colorByAtom = function (atomlist, colors) {
        ...
        for (var i in atomlist) {
            ...
            if (atom["forceColor"] != null) {
                atom.color = atom["forceColor"];
                atom.opacity = atom["forceOpacity"];
                atom.radius = atom["forceRadius"];
            }
        }
    ...
    GLmol.prototype.defineRepresentation = function () {
        ...
        //   this.addThings(this.modelGroup);
    ...
    GLmol.prototype.loadMolecule = function (repressZoom) {
        if (this.pdb) {
            this.loadMoleculeStr(repressZoom, this.pdb);
