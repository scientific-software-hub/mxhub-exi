...
var i;
    ...
    for (i in data){
        ...
        if (data[i].sessionStartDate){
            data[i].sessionStartDate = new Date(data[i].sessionStartDate);
        }
    }
...
DewarListSelectorGrid.prototype.getStore = function(){
    this.store = Ext.create('Ext.data.Store', {
        fields:['beamlineLocation', 
                'storageLocation','containerStatus','containerType','sessionStartDate','creationDate','beamLineOperator','shippingStatus','shippingName', 'barCode', 'beamlineName', 
                'dewarCode', 'dewarStatus', 'sampleChangerLocation', 'sampleCount', 'sessionStartDate', 'type'],
                         ...
                         { 
                             ...
                             renderer : function(grid, a, record){
                                     if (record.data.creationDate){
                                        return moment(record.data.creationDate).format('DD-MM-YYYY');
                                     }                                 
                            ...
                            { 
                                ...
                                renderer : function(grid, a, record){
                                      if (record.data.sessionStartDate){
                                        return moment(record.data.sessionStartDate).format('DD-MM-YYYY');
                                     }   
                                }
...
window.DewarListSelectorGrid = DewarListSelectorGrid;
